...
function LookupDashboard({ lookup, brand }: { lookup: PublicCustomerLookup; brand: { primary: string; secondary: string } }) {
  return <section className="border-t border-line p-6"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold uppercase tracking-normal text-slate-500">Lightweight Dashboard</p><h2 className="mt-1 text-xl font-semibold text-ink">Records for {lookup.email}</h2></div><span className="w-fit rounded-full px-3 py-1 text-xs font-semibold text-white" style={{ background: brand.secondary }}>{lookup.business.name}</span></div><div className="mt-5 grid gap-4 lg:grid-cols-3"><RecordGroup title="Bookings" records={lookup.bookings} empty="No matching bookings found." /><RecordGroup title="Invoices" records={lookup.invoices} empty="No matching invoices found." /><RecordGroup title="Quotes" records={lookup.quotes} empty="No matching quotes found." /></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><Placeholder title="Past Services" body={lookup.service_history_placeholder} /><Placeholder title="Recurring Services" body="Recurring service plan access is coming later." /></div><p className="mt-4 rounded bg-slate-50 p-3 text-xs leading-5 text-slate-500">{lookup.security_note}</p></section>;
}

function RecordGroup({ title, records, empty }: { title: string; records: PublicLookupRecord[]; empty: string }) {
  return <div className="rounded border border-line bg-slate-50 p-4"><p className="text-sm font-semibold text-ink">{title}</p><div className="mt-3 grid gap-2">{records.length === 0 ? <p className="text-sm text-slate-600">{empty}</p> : records.map((record) => <Link key={`${title}-${record.id}`} href={record.href} className="rounded border border-line bg-white p-3 text-sm font-semibold text-ink"><span className="break-words">{record.label}</span><span className="mt-1 block text-xs font-medium text-slate-500">{record.service_type ? `${record.service_type} · ` : ""}{record.scheduled_start ? `${formatDateTime(record.scheduled_start)} · ` : ""}{record.balance_due != null ? `Balance ${formatCurrency(record.balance_due)} · ` : ""}{record.status.replaceAll("_", " ")}</span></Link>)}</div></div>;
}
